...
<body class="bg-light">
  <main class="container-fluid">
      ...
      <div class="my-2 p-3 bg-warning bg-gradient bg-opacity-76 rounded-4 shadow-sm">
                        ...
                        <div class=" my-2">
                          <div class="row">
                              <div class="col"><h4>Bidder Data</h4></div>
                              <div class="col-md-auto"><a href='{{ route('bidder-data.create') }}' class="btn btn-primary rounded-3">Add Bidder data +</a></div>
                          </div>
                      </div>
          {{-- Alert group start--}}
                ...
                <div class="card-body">
                    <div class="table-responsive ">
                        <table class="table table-striped-row overflow-x-auto " id="dataTable" width="100%" cellspacing="0">
                            ...
                            <tbody>
                                @foreach ($dataPembeli as $dp)
                                <tr>
                                    <td class="col-1">{{ $loop->iteration}}</td>
                                    <td class="col-3">{{ $dp->name }}</td>
                                    <td class="col-2">{{ $dp->username }}</td>
                                    <td class="col-2">{{ $dp->email }}</td>
                                    <td align="middle" class="col-3"><a href='{{ route('bidder-data.edit', $dp->id) }}' class="btn btn-warning mt-2" style="--bs-btn-padding-y: .25rem; --bs-btn-padding-x: .5rem; --bs-btn-font-size: .75rem;">
                                      ...
                                      <form onsubmit="return confirm('Are you sure want to delete this bidder data?')" class="d-inline" action="{{ route('bidder-data.destroy', $dp->id) }}" method="post">
                                        @csrf
                                        @method('DELETE')
